...
# Настраиваем формат логов для отслеживания работы приложения
logging.basicConfig(
    level=logging.INFO,
...
class Settings:
    ...
    def validate(self) -> None:
        ...
        Выбрасывает ValueError, если какие-то ключи отсутствуют.
        """
        missing_keys = []
...
# Валидируем настройки при запуске
settings.validate()
...
class TopicRequest(BaseModel):
    ...
    Attributes:
        topic: Тема для генерации контента (обязательное поле)
        language: Язык новостей (опционально, по умолчанию 'en')
        max_news: Максимальное количество новостей для контекста (опционально)
    """
    topic: str = Field(
        ...,
        min_length=2,
...
class GeneratedContent(BaseModel):
    ...
    Attributes:
        title: Сгенерированный заголовок статьи
        meta_description: Мета-описание для SEO
        post_content: Основной текст статьи
        news_sources: Список использованных новостей
        generated_at: Время генерации контента
    """
...
def get_recent_news(
    ...
    # Формируем параметры запроса к Currents API
    params = {
        "language": language,
        "keywords": topic,
    ...
    try:
        # Выполняем GET-запрос к API с таймаутом
        ...
        # Формируем строку с новостями для контекста
        news_context = "\n".join([
...
def generate_title(topic: str, news_context: str) -> str:
    ...
    Args:
        topic: Тема статьи
        news_context: Контекст из актуальных новостей
...
Статья должна быть на русском языке."""
    ...
    response = openai_client.chat.completions.create(
        ...
        frequency_penalty=0.6  # Увеличивает разнообразие слов
    )
...
def generate_content(topic: str, language: str = "en", max_news: int = 5) -> GeneratedContent:
    ...
    Процесс:
    1. Получение актуальных новостей по теме
    2. Генерация заголовка на основе новостей
    3. Генерация мета-описания
    4. Генерация основного текста статьи

    Args:
        topic: Тема для генерации контента
        language: Язык новостей
        max_news: Максимальное количество новостей

    Returns:
        GeneratedContent: Объект с сгенерированным контентом
...
@app.get(
    ...
    description="Проверка работоспособности сервиса и получение базовой информации",
    tags=["Health Check"]
)
async def root() -> HealthResponse:
    ...
    Returns:
        HealthResponse: Статус сервиса и время
    """
...
async def health_check() -> HealthResponse:
    ...
    Используется для мониторинга и health checks в Kubernetes/Docker.
...
async def heartbeat() -> dict:
    """
    Простой heartbeat эндпоинт для проверки доступности.
...
@app.post(
    ...
    description="""
    Генерирует полноценную статью на заданную тему с использованием актуальных новостей.

    Процесс генерации:
    1. Поиск актуальных новостей по теме через Currents API
    2. Генерация SEO-оптимизированного заголовка
    3. Создание мета-описания
    4. Генерация полного текста статьи с учётом найденных новостей
    """,
...
async def generate_post_api(request: TopicRequest) -> GeneratedContent:
    ...
    Raises:
        HTTPException: При ошибках валидации или генерации
    """
    logger.info(f"📨 Получен запрос на генерацию контента: тема='{request.topic}'")
...
@app.get(
    ...
    description="Возвращает список актуальных новостей по заданной теме",
    tags=["News"]
)
async def get_news(
    topic: str,
    ...
    """
    Получает актуальные новости по теме без генерации контента.
    ...
    Returns:
        dict: Список найденных новостей
    """
...
async def http_exception_handler(request, exc: HTTPException):
    ...
    Форматирует ответ об ошибке в единый формат.
    """
    return {
        "error": "HTTPException",
        "detail": exc.detail,
...
async def startup_event():
    ...
    Инициализация ресурсов и проверка подключений.
    """
...
async def shutdown_event():
    ...
    Освобождение ресурсов и завершение подключений.
    """
